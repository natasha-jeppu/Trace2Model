...
def nfa_traverse(model,trace, vis=[],find_vis=False):
	...
	for i in range(len(trace)):
		if trace[i] == 1:
			state = [1]
		# found = list of states reached: 
		# 	if len(found) > 1 then non-deterministic transition
...
def nfa_to_dfa(nfa):
	def find_eq_states_to_merge(full_trans):
		...
		for o1 in outgoing:
			...
			for o2 in outgoing:
				...
				if same_out:
					invalid_merge = False
					if full_trans[o1] != full_trans[o2]:
						for e_ind in outgoing[o2]:
							if full_trans[o2][e_ind] != full_trans[o1][e_ind]:
								invalid_merge = True
								break
	...
	while old_full_trans != full_trans:
		...
		to_be_merged = find_eq_states_to_merge(full_trans)
		if to_be_merged:
			full_trans = merge_states(full_trans, to_be_merged)
...
def parse_args():
    ...
    required_parse.add_argument('-i','--input_file', metavar = 'INPUT_FILENAME', required = True,
            help='Input trace file for incremental model learning')
...
def make_model(full_events, model, var, hyperparams, num_states, input_dict):
	...
	if incr_SAT:
		...
		for i in range(num_of_ind):
			ind = random.randint(10, round(0.75*len(full_events)))
			ind_list.append(ind)
...
def main():
	...
	var = {'incr': 0, 'events_tup_to_list': events_tup_to_list, 'o_event_uniq': o_event_uniq, 'o_event_id': o_event_id, 'org_trace': []}
	input_dict = {'event_id':[], 'seq_input_uniq':[], 'event_uniq':[], 'len_seq':len_seq, 'seq_input_uniq_ce':[]}
	get_seq_input_uniq(var)

	model_gen = []
	i = 1
	# events_tup_to_list = set of traces
	for event_list in events_tup_to_list:
		print("\n\n\n******************************************** TRACE:" + str(i))
		# call make_model() to generate a new model
		# model_gen - stores the model generated so far
		# event_list - new trace
	...
	while(True):
		...
		if f:
			...
			end_time = time.time()
			print('\n\nTime taken: ' + str(end_time - start_time))
			plot_model(final_model,input_dict_store,num_states,hyperparams,do_dfa != 'y')
			break
		else:
			print(colored("\n[ERROR] Missing behavior",'red'))
			print(colored(trace,'red'))
